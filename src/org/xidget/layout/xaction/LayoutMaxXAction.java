 ...
 *   - Attach the right side of the form to all the xidgets last to first (order of precedence).
 */
public class LayoutMaxXAction extends AbstractLayoutAction
{
  /* (non-Javadoc)
  ...
  protected void layout( IContext context, IXidget parent, List<IXidget> children, Margins margins, int spacing)
  {
    ...
    {
      IComputeNode node = getComputeNode( children.get( i), Type.right);
      max.addDependency( node);
    }
    
    IComputeNode form = getParentNode( parent, Type.right);
    form.addDependency( new OffsetNode( max, margins.x1));
  }
}
